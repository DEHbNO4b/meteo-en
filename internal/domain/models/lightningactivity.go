...
func (la *LightningActivity) SetMaxPozitiveSignal(s int) {
	if s > la.maxPozitiveSignal {
		la.maxPozitiveSignal = s
	}
}

func (la *LightningActivity) SetMaxNegativeSignal(s int) {
	if s < la.maxNegativeSignal {
		la.maxNegativeSignal = s
	}
}

func (la *LightningActivity) SetPozSignal(s int) {
	// la.signal = (la.signal + s) / 2
	la.pozitiveSignal = s
}

func (la *LightningActivity) SetNegSignal(s int) {
	// la.signal = (la.signal + s) / 2
	la.negativeSignal = s
}
...
func (la *LightningActivity) MaxPozSig() int {
	return la.maxPozitiveSignal
}

func (la *LightningActivity) MaxNegSig() int {
	return la.maxNegativeSignal
}

func (la *LightningActivity) PozSig() int {
	return la.pozitiveSignal
}

func (la *LightningActivity) NegSig() int {
